...
function loanCard(l) {
    ...
    return `
    <div style="background:var(--surface);border:1.5px solid var(--border);border-radius:22px;overflow:hidden;">
...
function investCard(c) {
    ...
    return `
        <a href="/?p=chat&id=${c.id}" style="background:var(--surface);border:1.5px solid ${hasUnread ? 'var(--accent)' : 'var(--border)'};border-radius:22px;overflow:hidden;text-decoration:none;color:inherit;display:block;transition:border-color .12s,box-shadow .12s;" onmouseenter="this.style.boxShadow='0 4px 14px rgba(0,0,0,.08)'" onmouseleave="this.style.boxShadow='none'">
            <div style="padding:14px 18px 12px;display:flex;align-items:center;gap:12px;border-bottom:1px solid var(--surface-3)">
                <div style="width:44px;height:44px;border-radius:50%;background:var(--surface-3);border:1.5px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:.6875rem;font-weight:800;color:var(--ink-2);text-transform:uppercase;flex-shrink:0;">${ini}</div>
