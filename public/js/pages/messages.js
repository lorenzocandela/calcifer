...
function buildRow(c, i, total) {
    ...
    return `
        <div class="chat-row" data-id="${c.id}" style="position:relative;overflow:hidden;border-radius:20px;background:#fff;border:1.5px solid var(--border)">
            <div class="chat-delete-bg" style="position:absolute;right:0;top:0;bottom:0;width:80px;background:#ef4444;display:flex;align-items:center;justify-content:center;transform:translateX(100%);transition:transform .2s;border-radius:0 18px 18px 0;">
                <span class="ms" style="color:#fff;font-size:22px">delete</span>
            </div>

            <a href="/?p=chat&id=${c.id}" class="chat-row-inner" style="display:flex;align-items:center;gap:14px;padding:14px 16px;background:#fff;text-decoration:none;color:inherit;transition:background .1s;position:relative;z-index:1;transform:translateX(0);will-change:transform;border-radius:18px;">
