...
async function load() {
    ...
    try {
        ...
        list.innerHTML = `<div style="text-align:center;padding:60px 16px;color:var(--ink-3)"><span class="ms" style="font-size:44px">error</span><p style="margin-top:8px;font-weight:600">${e.message}</p></div>`;
    }
}
...
function loanCard(l) {
    ...
    return `
      <div class="feed-card" data-id="${l.id}" style="
        background:var(--surface);
        border:1.5px solid var(--border);
        border-radius:22px;overflow:hidden;cursor:pointer;
        transition:border-color .14s,box-shadow .14s;
      " onmouseenter="this.style.borderColor='var(--border-md)';this.style.boxShadow='0 4px 16px rgba(0,0,0,.07)'" onmouseleave="this.style.borderColor='var(--border)';this.style.boxShadow='none'">
      ...
      ">
        ...
        <div style="overflow-y:auto;flex:1;padding:20px 20px 32px">
          ...
          <button style="
            ...
            color:var(--ink-2);font-size:.9375rem;font-weight:700;
            cursor:pointer;font-family:var(--sans);
          " id="__close_sheet">Chiudi</button>
        </div>
      </div>`;
