...
const LEVELS = [
    { max:125, img:'1.png', label:'Scintilla', desc:'La tua prima fiamma',      color:'#f59e0b', bg:'#fef3c7', shadow:'rgba(245,158,11,.28)' },
    { max:250, img:'2.png', label:'Fiammella', desc:'Stai prendendo fuoco',     color:'#f97316', bg:'#ffedd5', shadow:'rgba(249,115,22,.28)' },
    { max:375, img:'2.png', label:'Fuoco',     desc:'Il sistema si fida di te', color:'#ef4444', bg:'#fee2e2', shadow:'rgba(239,68,68,.28)'  },
    { max:500, img:'1.png', label:'Calcifer',  desc:'Credibilità massima',      color:'#be123c', bg:'#ffe4e6', shadow:'rgba(190,18,60,.28)'  },
];
...
function buildGauge(score, cid, sid) {
    ...
    el.innerHTML = `
      ...
      <!-- step indicator (usa classi CSS tue) -->
      <div class="step-indicator">
        ${LEVELS.map((l, i) => {
          ...
          return `
            ...
            ${i < LEVELS.length - 1 ? `
            <div class="step-line">
              <div class="step-line-fill" data-fill="${done ? 100 : 0}" style="background:${done ? l.color : 'transparent'}"></div>
            </div>` : ''}`;
        }).join('')}
      </div>`;
...
async function initDashboard() {
    try {
        ...
        document.getElementById('dash-stats').innerHTML = [
            { icon:'request_quote', label:'Richieste',  val: h.total_requests },
            { icon:'savings',       label:'Operazioni', val: c.total_investments },
        ].map(s => `
          <div class="dash-stat">
